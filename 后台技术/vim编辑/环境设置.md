...
```shell
# 基础配置
set enc=utf-8                         # 设置编辑文件的内码为UTF-8
set nocompatible                      # 设置Vim不需要和vi兼容
source $VIMRUNTIME/vimrc_example.vim  # 导入VIm的示例配置（会打开一些有用的选项，如语法加亮、搜索加亮、命令历史、记住上次的文件位置，等）
...
上面的环境设置，会默认打开备份文件与撤销文件自动保存。下面我们需要关闭备份文件，以及将撤销文件自定义用户目录，更加安全
...
# 备份文件设置
set nobackup                          # 去除备份文件
set undodir=~/.vim/undodir            # 将撤销文件保存到个人的特定目录

# 检查自定义的备份撤销目录是否存在，不存在则创建
...
# 备份文件设置
set nobackup                          # 去除备份文件
set undodir=~\vimfiles\undodir        # 将撤销文件保存到个人的特定目录
